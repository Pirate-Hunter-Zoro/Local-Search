# About
This is a repository containing my implementation of various local-search methods to approximate function maxima along a domain. To see various notebooks in action, you'll want to run each '.ipynb' notebook and observe the output. My instructions apply only to Mac and Linux users - Windows users are unfortunately on their own in getting this code to run on their machines!
...
1. Navigate to a folder on your computer where you would like to clone this project via the command line and run the following command:<br>
   ```git clone https://github.com/Pirate-Hunter-Zoro/Local-Search.git```<br>
This will create a folder called "Local-Search" in your current folder. 
